...
namespace SmartERP.Inventory.Endpoints
{
    ...
    using MyRepository = Repositories.DamagedProductsRepository;
    using MyRow = Entities.DamagedProductsRow;
    using Serenity.Reporting;
    using Serenity.Web;
    using System;

    [Route("Services/Inventory/DamagedProducts/[action]")]
    [ConnectionKey(typeof(MyRow)), ServiceAuthorize(typeof(MyRow))]
   
    public class DamagedProductsController : ServiceEndpoint
    {
        ...
        public FileContentResult ListExcel(IDbConnection connection, ListRequest request, [FromServices] IExcelExporter exporter)
        {
            ...
            var bytes = exporter.Export(data, typeof(Columns.DamagedProductsColumns), request.ExportColumns);

            return ExcelContentResult.Create(bytes, "DamagedProductsList_" +
