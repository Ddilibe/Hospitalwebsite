...
namespace SmartERP.Quotations.Endpoints
{
    using Serenity;
    using Serenity.Data;
    ...
    using MyRepository = Repositories.QuotationsRepository;
    using MyRow = Entities.QuotationsRow;
    using Serenity.Reporting;
    using Serenity.Web;
    using System;

    [Route("Services/Quotations/Quotations/[action]")]
    [ConnectionKey(typeof(MyRow)), ServiceAuthorize(typeof(MyRow))]
    
    public class QuotationsController : ServiceEndpoint
    {
        ...
        public FileContentResult ListExcel(IDbConnection connection, ListRequest request, [FromServices] IExcelExporter exporter)
        {
            ...
            var bytes = exporter.Export(data, typeof(Columns.QuotationsColumns), request.ExportColumns);

            return ExcelContentResult.Create(bytes, "QuotationsInvoicesList_" +
